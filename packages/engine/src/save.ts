/**
 * Save / load for game state.
 *
 * A save is a plain, versioned, JSON-serializable snapshot of a `GameState` with
 * the `board` omitted — the board is large and fully reconstructable from
 * `options.boardMode` via `getBoard()`, so it never needs to travel in the file.
 * Randomness round-trips as `rngSeed` + `rngCursor` (see rng.ts).
 *
 * Two layers:
 ...
 *    `GameState` ready to hand to `applyAction`. This is what lets a save file be
 *    written by hand to pit the game into a specific state for testing.
 */
...
/** Raised when a save cannot be loaded. Catchable and carries a clear message. */
...
/**
 * Snapshot a live game to a canonical, JSON-serializable save. The board is
 * dropped (rebuilt from `options.boardMode` on load); everything else is copied
...
export function serializeGame(state: GameState): SaveGame {
  ...
  return { version: SAVE_VERSION, ...clone(rest) };
}
...
/**
 * Rebuild a `GameState` from a (possibly partial, hand-authored) save. Fills
 * defaults, reconstructs the board, and validates; throws `SaveError` on any
 * inconsistency with a message that names the offending field/value.
 */
export function deserializeGame(input: SaveGameInput): GameState {
  if (!input || typeof input !== "object") throw new SaveError("save is not an object");

  if (input.version !== undefined && input.version > SAVE_VERSION)
    throw new SaveError(`save version ${input.version} is newer than supported (${SAVE_VERSION})`);

  // --- board ---------------------------------------------------------------
  if (!input.options || typeof input.options !== "object")
    throw new SaveError("missing options");
  const boardMode: BoardMode = input.options.boardMode ?? "world";
  const board = getBoard(boardMode);
  if (!board) throw new SaveError(`unknown boardMode "${boardMode}"`);
  ...
  if (!Array.isArray(input.players) || input.players.length < 2)
    throw new SaveError("need at least 2 players");
  const seen = new Set<PlayerId>();
  const players: Player[] = input.players.map((p, i) => {
    if (!p || !p.id) throw new SaveError(`player ${i} is missing an id`);
    if (seen.has(p.id)) throw new SaveError(`duplicate player id "${p.id}"`);
    seen.add(p.id);
    if (!p.name || !p.color || !p.kind)
      throw new SaveError(`player "${p.id}" needs name, color and kind`);
    return {
      id: p.id,
      name: p.name,
      color: p.color,
  ...
  if (!firstAlive) throw new SaveError("every player is eliminated");
  const activePlayer = input.activePlayer ?? firstAlive.id;
  const active = players.find((p) => p.id === activePlayer);
  if (!active) throw new SaveError(`activePlayer "${activePlayer}" is not a player`);
  if (active.eliminated) throw new SaveError(`activePlayer "${activePlayer}" is eliminated`);

  // --- territories (must match the board exactly) --------------------------
  if (!input.territories || typeof input.territories !== "object")
    throw new SaveError("missing territories");
  const boardIds = Object.keys(board.territories);
  const saveIds = new Set(Object.keys(input.territories));
  const unknown = [...saveIds].filter((id) => !board.territories[id]);
  if (unknown.length) throw new SaveError(`unknown territory ids: ${unknown.join(", ")}`);
  const missing = boardIds.filter((id) => !saveIds.has(id));
  if (missing.length)
    throw new SaveError(
  ...
  for (const id of boardIds) {
    ...
    if (!t || typeof t.armies !== "number")
      throw new SaveError(`territory "${id}" needs a numeric armies count`);
    const owner = t.owner ?? null;
    if (owner !== null && !playerIds.has(owner))
      throw new SaveError(`territory "${id}" owned by unknown player "${owner}"`);
    if (owner === null && t.armies !== 0)
      throw new SaveError(`unowned territory "${id}" must have 0 armies`);
    if (owner !== null && t.armies < 1)
      throw new SaveError(`owned territory "${id}" must have at least 1 army`);
    territories[id] = { owner, armies: t.armies };
  }

  // --- phase ---------------------------------------------------------------
  const phase = input.phase ?? "reinforce";
  if (!PHASES.includes(phase)) throw new SaveError(`unknown phase "${phase}"`);
  ...
  const validateCards = (cards: Card[], where: string) => {
    for (const c of cards)
      if (c.territory != null && !board.territories[c.territory])
        throw new SaveError(`card "${c.id}" in ${where} pictures unknown territory "${c.territory}"`);
  };
  ...
  if (pendingOccupation) {
    ...
    if (!board.territories[from] || !board.territories[to])
      throw new SaveError("pendingOccupation references an unknown territory");
    if (!board.territories[from].neighbours.includes(to))
      throw new SaveError("pendingOccupation from/to are not adjacent");
    if (territories[from].owner !== activePlayer)
      throw new SaveError("pendingOccupation.from must be owned by the active player");
  }
...
/** Load a `GameState` from a JSON string produced by `saveToJSON` or hand-written. */
export function loadFromJSON(json: string): GameState {
  ...
  return deserializeGame(parsed as SaveGameInput);
}
